...
export const updateProductAction = async (productId: string, product: InsertProducts) => {
    createInsertSchema(products).parse(product);
    const insertedProduct = await db.update(products).set(product).where(eq(products.id, productId)).returning();
