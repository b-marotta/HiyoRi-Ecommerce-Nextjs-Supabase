...
const CollectionRouteQuery = gql(/* GraphQL */ `
    query CollectionRouteQuery($collectionSlug: String) {
        collectionsCollection(filter: { slug: { eq: $collectionSlug } }, orderBy: [{ order: DescNullsLast }], first: 1) {
            edges {
                node {
                    title
                    label
    ...
    return (
        <Shell>
            ...
            <Suspense fallback={<SearchProductsGridSkeleton />}>
                <SearchProductsInifiteScroll collectionId={data.collectionsCollection.edges[0].node.id} />
            </Suspense>
        </Shell>
    );
}
