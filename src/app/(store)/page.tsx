...
import { CollectionCardFragment, CollectionsCard, CollectionsCardSkeleton } from "@/features/collections";
...
const LandingRouteQuery = gql(/* GraphQL */ `
    query LandingRouteQuery($user_id: UUID) {
        products: productsCollection(filter: { featured: { eq: true } }, first: 4, orderBy: [{ created_at: DescNullsLast }]) {
            edges {
                node {
                    id
                    ...ProductCardFragment
        ...
        collectionScrollCards: collectionsCollection(first: 6, orderBy: [{ order: DescNullsLast }]) {
            edges {
                node {
                    id
...
export default async function Home() {
    ...
    return (
        <main>
            ...
            <Shell>
                {data.products && data.products.edges ? <ProductSubCollectionsCircles collections={data.collectionScrollCards.edges} /> : null}

                {data.products && data.products.edges ? <FeaturedProductsCards products={data.products.edges} /> : null}
        ...
        <section className="w-full h-screen md:h-[800px] mx-auto flex justify-center">
            ...
            <div className="container absolute py-8 h-screen md:h-[800px] w-full">
                <div className="flex flex-col justify-center z-30 h-full">
                    <p className="text-sm md:text-md uppercase tracking-widest text-white ">hugolam</p>
                    ...
                    <div className="flex space-x-4 mt-5 max-w-screen">
                        ...
                        <Link
                            ...
                            target="_blank"
                            className={cn(buttonVariants({ variant: "default", size: "lg" }), "border-2 border-primary text-white rounded px-8 py-3 ", "md:px-16 md:py-6")}
                        >
                            View the Code
                        </Link>
                    </div>
...
function FeaturedProductsCards({ products }: FeaturedProductsCardsProps) {
    return (
        <section className="container mt-12">
            <div className="">
                <h2 className="font-semibold text-2xl md:text-3xl mb-1 md:mb-3">Featured Products</h2>
                <p className="max-w-4xl text-sm md:text-md leading-[1.5] tracking-[-2%] mb-2">
                    Ideas to help Bring Home to Life based on your recently viewed products. Share your space on Instagram and tag @Penpengrian
                </p>
            </div>
    ...
    return (
        <section className="relative lg:space-x-5 space-y-5 lg:space-y-0 grid grid-cols-1 lg:grid-cols-3 max-h-[840px]">
            <div className="relative col-span-2 w-full h-[840px]">
                <Image src={keytoUrl("public/zPiCx79oGe5X4rVBLg0Ss.jpeg")} width={1080} height={1080} className="object-cover w-full h-full" alt="1" />
                <div className="bg-zinc-800/20 flex justify-center items-center flex-col absolute w-full h-full top-0 left-0 text-white">
                    ...
                    <Link className={cn(buttonVariants({ size: "lg" }), "text-xl py-8 px-10")} href={"/collections/bathroom"}>
                        DiscoverNow
                    </Link>
                </div>
            </div>
            ...
            <div className="flex flex-col w-full space-y-5 h-[840px]">
                <div className="relative w-full h-[340px]">
                    <Image src={keytoUrl("public/E2MWE99uGyOZLd76UEixy.jpeg")} width={800} height={900} className="object-cover w-full h-full" alt="1" />
                </div>

                <div className="relative overflow-hidden">
                    <Image src={keytoUrl("public/YPO3VwJvjvlkWzNtIv9FS.jpeg")} width={800} height={900} className="object-cover w-full h-full" alt="1" />
                </div>
            </div>
        </section>
    );
    ...
    return (
        <section className="max-w-[1920px] mx-auto h-[620px] md:h-[580px] bg-[#FFF8EE] grid grid-cols-12 my-16">
            <div className="relative w-full h-[340px] md:h-[580px] col-span-12 md:col-span-8 overflow-hidden">
                <Image src={"/assets/cutingcardImage.jpg"} alt="" fill className="object-cover object-center" />
            </div>

            <div className="col-span-12 md:col-span-4 pb-6 md:py-20 px-6 md:px-16">
                <h2 className="text-xl md:text-3xl font-semibold mb-3">Less is More. Minimal.</h2>
                <p className="text-xs leading-[1.5] md:text-lg tracking-tight mb-5 md:mb-12 text-left max-w-md">
                    We believe no one should have to choose between the quality they want, and the price they can afford. That’s why we make sure our products stand up to only the
                    highest quality and sustainability standards - and produce them in a way that keeps great design affordable for everyone.
                </p>
