...
export async function POST(request: Request) {
    ...
    try {
        const productsQuantity = await mergeProductDetailsWithQuantities(data.orderProducts);
        ...
        const insertedOrder = await db
            .insert(orders)
            .values({
                user_id: !data.guest ? (await supabase.auth.getUser()).data.user.id : null,
                currency: "cad",
                amount: `${amount}`,
...
const calcSubtotal = (productsQuantity: (SelectProducts & { quantity: number })[]) =>
    productsQuantity.reduce((acc, cur) => {
        return acc + cur.quantity * parseFloat(cur.price);
    }, 0);

const mergeProductDetailsWithQuantities = async (orderProducts: OrderProducts): Promise<(SelectProducts & { quantity: number })[]> => {
