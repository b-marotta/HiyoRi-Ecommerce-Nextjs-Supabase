...
export async function POST(request: NextRequest) {
    ...
    const uploadResponse = await Promise.all(
        Object.entries(data).map(async ([index, file]) => {
            ...
            try {
                ...
                if (s3Response) {
                    const insertedMedia = await db.insert(medias).values({ alt: file.name, key: params.Key }).returning();
    ...
    return statusCode >= 300 ? NextResponse.json({ message: errorMessage }, { status: statusCode }) : NextResponse.json(uploadResponse, { status: statusCode });
}
