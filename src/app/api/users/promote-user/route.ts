...
export async function POST(request: NextRequest) {
    ...
    if (!validate)
        return NextResponse.json({ message: 'Error, Data validation failed.' }, { status: 500 })
    ...
    const { data: updatedUser, error } = await client.auth.admin.updateUserById(data.userId, {
