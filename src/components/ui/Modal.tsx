...
function Modal({ header, containerClassName, children }: Props) {
    return (
        <section className={cn("fixed z-50 inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full")}>
            <Card
                className={cn(
            ...
            >
                <CardHeader className="p-0 mb-3 md:mb-5">
                    <h1 className="font-semibold text-lg leading-5 tracking-tight">{header}</h1>
                    <CloseButton />
                </CardHeader>

                <CardContent className="relative p-0 mb-5 overflow-hidden">{children}</CardContent>
            </Card>
        </section>
    );
}
