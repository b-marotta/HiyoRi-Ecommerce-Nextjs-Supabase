...
function CartItemCard({ product, disabled, addOneHandler, minusOneHandler, removeHandler, quantity }: CartItemCardProps) {
    return (
        <Card className="flex items-center justify-between gap-x-6 gap-y-8 px-5 py-3 shadow-none border-0 border-b">
            <CardContent className="relative p-0 mb-5 overflow-hidden ">
                <Image src={keytoUrl(product.featuredImage.key)} alt={product.featuredImage.alt} width={150} height={150} className="aspect-square object-cover" />
            </CardContent>
            ...
            <CardHeader className="p-0 mb-3 md:mb-5 grow max-w-lg">
                ...
                <CardDescription className="grow line-clamp-2">{product.description}</CardDescription>

                <QuantityInput value={quantity} addOneHandler={addOneHandler} minusOneHandler={minusOneHandler} disabled={disabled} />
            </CardHeader>
