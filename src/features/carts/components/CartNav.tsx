...
const GuestCart = () => {
    ...
    const productCountStorage = useMemo(() => calcProductCountStorage(cart), [cart]);
...
const UserCartNav = ({ currentUser }: { currentUser: User }) => {
    ...
    const productCount = useMemo(() => (carts?.edges || []).reduce((acc, cur) => acc + cur.node.quantity, 0), [carts.edges]);
