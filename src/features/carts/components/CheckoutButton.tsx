...
function CheckoutButton({ order, guest, ...props }: CheckoutButtonProps) {
    ...
    return (
        <Button {...props} className={cn("w-full", props.className)} onClick={onClickHandler} disabled={isLoading}>
            {isLoading ? "Loading ...  " : "Check out"}
            {isLoading && <Spinner className="ml-3 h-4 w-4 animate-spin" aria-hidden="true" />}
        </Button>
    );
}
