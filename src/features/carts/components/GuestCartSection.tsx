...
function GuestCartSection() {
    ...
    const subtotal = useMemo(() => calcSubtotal({ prdouctsDetails: data, quantity: cartItems }), [data, cartItems]);
                    ...
                    <div className="col-span-12 md:col-span-9 max-h-[420px] md:max-h-[640px] overflow-y-auto">
                        {data.productsCollection.edges.map(({ node }) => (
                            <CartItemCard
                                ...
                                addOneHandler={() => addOneHandler(node.id, cartItems[node.id].quantity)}
                                minusOneHandler={() => minusOneHandler(node.id, cartItems[node.id].quantity)}
...
export const LoadingCartSection = () => (
    <section className="grid grid-cols-12 gap-x-6 gap-y-5" aria-label="Loading Skeleton">
        <div className="col-span-12 md:col-span-9 space-y-8">
            {[...Array(4)].map((_, index) => (
                <div className="flex items-center justify-between gap-x-6 gap-y-8 border-b p-5" key={index}>
...
const calcSubtotal = ({ prdouctsDetails, quantity }: { prdouctsDetails: DocumentType<typeof FetchGuestCartQuery>; quantity: CartItems }) => {
    const productPrices = prdouctsDetails && prdouctsDetails.productsCollection.edges ? prdouctsDetails.productsCollection.edges : [];

    if (!productPrices.length) return 0;

    return productPrices.reduce((acc, cur) => acc + quantity[cur.node.id].quantity * cur.node.price, 0);
};
