...
function useCartActions(user: User | null, productId: string) {
    ...
    const authAddOrUpdateProduct = async (quantity: number) => {
        const existedProduct = data?.cartsCollection.edges.find(({ node }) => node.product_id === productId);
        try {
            let res;
            if (!existedProduct) {
                res = await addToCart({
    ...
    const addProductToCart = (quantity: number) => (!user ? guestAddProduct(quantity) : authAddOrUpdateProduct(quantity));
