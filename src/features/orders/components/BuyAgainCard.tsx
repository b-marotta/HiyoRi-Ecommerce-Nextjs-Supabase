                ...
                {products.map(({ node }) => (
                    <div key={node.id} className="grid grid-cols-5">
                        <div className="relative col-span-2">
                            <Image src={keytoUrl(node.featuredImage.key)} alt={node.featuredImage.alt} className="w-[80px] h-[80px] object-cover" width={80} height={80} />
                        </div>

                        <div className="col-span-3 ">
                            <Link href={"/shop/" + node.slug} className="text-blue-500 line-clamp-3">
                                {node.name}
                            </Link>
