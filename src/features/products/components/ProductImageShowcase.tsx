...
function ProductImageShowcase({ data }: ProductImageShowcaseProps) {
    const allImages = [data.featuredImage, ...(data.images?.edges.map(({ node }) => node.media) || [])];
        ...
        <section className="flex md:flex-row flex-col items-center gap-x-8 gap-y-5">
            ...
            <div className="relative order-2 overflow-x-auto w-full md:w-[100px] h-full">
                ...
                <div className="md:hidden block">
                    <button onClick={prevImage} className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-gray-800 text-white p-2">
                        <Icons.chevronLeft />
                    </button>

                    <button onClick={nextImage} className="absolute right-0 top-1/2  md:top-unset transform -translate-y-1/2 bg-gray-800 text-white p-2">
                        <Icons.chevronRight />
                    </button>
                </div>
            </div>
