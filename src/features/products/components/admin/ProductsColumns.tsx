...
const ProductsColumns: ColumnDef<{
...
}>[] = [
    {
        ...
        cell: ({ row }) => {
            ...
            return (
                <Link href={`/admin/products/${product.id}`} className="text-center font-medium capitalize px-3 hover:underline">
                    {product.name}
                </Link>
            );
        },
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            return <div className="font-medium">{product.collections ? product.collections.label : "-"}</div>;
        },
    },
    {
            ...
            return (
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="start" className="flex flex-col items-start">
                        ...
                        <Link href={`/admin/products/${product.id}`} className={buttonVariants({ variant: "ghost" })}>
                            Edit Product
                        </Link>
...
const DeleteCategoryDialog = ({ categoryId }: { categoryId: string }) => {
    ...
    return <DeleteDialog onClickHandler={onClickHandler} title="Delete Proejct" actionLabel="Delete" />;
};
