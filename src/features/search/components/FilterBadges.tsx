        ...
        <section className="gap-x-10 md:flex hidden">
            {query.search && (
                <Badge className="px-3 py-2 gap-x-3">
                    {`Search: ${query.search}`}
                    <button onClick={() => router.push(pathname + "?" + onDeleteHandler("search"))} className={cn("rounded-full")}>
            ...
            {query.priceRange && (
                <Badge className="px-3 py-2 gap-x-3">
                    ...
                    <button onClick={() => router.push(pathname + "?" + onDeleteHandler("price_range"))} className={cn("rounded-full")}>
                        <Icons.close width={15} height={15} />
                    </button>
                </Badge>
            )}
                ...
                collections.map((collection, index) =>
                    query.collections.includes(collection.id) ? (
                        <Badge key={index} className="px-3 py-2 gap-x-3">
                            ...
                            <button
                                onClick={() => {
                                    const deletedcollections = query.collections.filter((c) => c !== collection.id);
                                    router.push(
                                        pathname +
                                            "?" +
                                            onDeleteHandler(
                                                "collections",
                                                deletedcollections.length > 0 ? JSON.stringify(query.collections.filter((c) => c !== collection.id)) : undefined
                                            )
                                    );
                                }}
