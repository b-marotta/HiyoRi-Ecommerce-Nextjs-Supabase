...
function FilterSelections({ collectionsSection, shopLayout = true }: Props) {
    ...
    useEffect(() => {
        ...
        const collections = (JSON.parse(searchParams.get("collections")) as string[]) ?? [];
        const sort = searchParams.get("sort") ?? undefined;
        const search = searchParams.get("search") ?? undefined;

        setQuery({
            sort: sort ? SortEnum[sort] : undefined,
            priceRange: range && range.length === 2 ? [parseInt(range[0]), parseInt(range[1])] : undefined,
            collections,
            search,
        });
    }, [searchParams]);
    ...
    React.useEffect(() => {
        ...
        if (query.priceRange !== undefined && !(query.priceRange[0] === 0 && query.priceRange[1] === 10000))
            startTransition(() => {
    ...
    const collectionChangeHandler = (collectionId: string) => {
        ...
        if (oldValue.includes(collectionId)) {
            ...
            router.push(pathname + "?" + createQueryString("collections", JSON.stringify(collections)));
        } else {
            const collections = [...oldValue, collectionId];
            setQuery({ ...query, collections });
            router.push(pathname + "?" + removeQueryString("collections", collections.length > 0 ? JSON.stringify(collections) : undefined));
        }
    };
        ...
        <>
            <section className="justify-between items-center hidden md:flex">
                <div className="flex gap-x-5 items-center">
                    <span>Filter:</span>
                    {shopLayout && (
                        <CollectionsSelection className="flex items-center" value={query.collections} onCheckedChange={collectionChangeHandler} selections={collectionsSection} />
                    )}
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent className="p-5 max-w-xl">
                            ...
                            <PriceRange
                                ...
                                onValueChange={(priceRange) => setQuery({ ...query, priceRange })}
                ...
                <SheetContent className="w-full">
                    <SheetHeader>
                        ...
                        <SheetDescription className="flex flex-col items-start">
                            {shopLayout && (
                                <div className="grid">
                                    <label className="text-primary font-semibold text-left">Collections</label>
                            ...
                            <div className="grid">
                                <label className="text-primary font-semibold text-left">Price Range</label>
                                ...
                                <PriceRange
                                    ...
                                    onReset={() => {
                                        ...
                                        router.push(pathname + "?" + removeQueryString("price_range"));
                                    }}
                                />
                            </div>
            ...
            <FilterBadges query={query} collections={collectionsSection} onDeleteHandler={removeQueryString} />
        </>
    );
}
