...
function FilterSelections({ collectionsSection, shopLayout = true }: Props) {
    ...
    useEffect(() => {
        ...
        setQuery({
            ...
            priceRange:
                range && range.length === 2 ? [parseInt(range[0]), parseInt(range[1])] : undefined,
            collections,
            search,
        })
    }, [searchParams])
    ...
    const collectionChangeHandler = (collectionId: string) => {
        ...
        if (oldValue.includes(collectionId)) {
            ...
            router.push(
                pathname + '?' + createQueryString('collections', JSON.stringify(collections)),
            )
        } else {
            ...
            <section className="hidden items-center justify-between md:flex">
                <div className="flex items-center gap-x-5">
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent className="max-w-xl p-5">
                ...
                <SheetContent className="w-full">
                    <SheetHeader>
                        ...
                        <SheetDescription className="flex flex-col items-start">
                            {shopLayout && (
                                <div className="grid">
                                    <label className="text-left font-semibold text-primary">
                                        Collections
                                    </label>
                                    <CollectionsSelection
                                        className="flex items-center"
