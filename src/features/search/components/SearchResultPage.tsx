...
const ProductSearch = gql(/* GraphQL */ `
    query Search($search: String, $lower: BigFloat, $upper: BigFloat, $collections: [String!], $first: Int!, $after: Cursor, $orderBy: [productsOrderBy!]) {
        productsCollection(
            filter: { and: [{ name: { ilike: $search } }, { price: { gt: $lower, lt: $upper } }, { collection_id: { in: $collections } }] }
            first: $first
            after: $after
            orderBy: $orderBy
        ) {
...
const SearchResultPage = ({ variables, onLoadMore, isLastPage }: { variables: SearchQueryVariables; onLoadMore: (cursor: string) => void; isLastPage: boolean }) => {
                ...
                <>
                    ...
                    {isLastPage && products.pageInfo.hasNextPage && (
                        <div className="w-full flex justify-center items-center mt-3">
                            <Button onClick={() => onLoadMore(products.pageInfo.endCursor)}>load more</Button>
                        </div>
                    )}
                </>
            )}
