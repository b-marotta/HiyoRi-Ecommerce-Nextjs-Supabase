...
const UsersColumns: ColumnDef<User>[] = [
    {
        ...
        cell: ({ row }) => {
            ...
            return (
                <Link href={`/admin/users/${user.id}`} className="text-center font-medium capitalize px-3">
                    {user.id}
                </Link>
            );
        },
            ...
            return (
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="start" className="flex flex-col items-start">
                        ...
                        <Link href={`/admin/products/${user.id}`} className={buttonVariants({ variant: "ghost" })}>
                            Edit User
                        </Link>
                        <Button onClick={() => promoteAdminHandler(user.id)}>Promote to Admin</Button>
                    </DropdownMenuContent>
                </DropdownMenu>
            );
        },
