...
const AddProductToWishList = gql(/* GraphQL */ `
    mutation AddProductToWishList($productId: String, $userId: UUID) {
        insertIntowishlistCollection(objects: { user_id: $userId, product_id: $productId }) {
            affectedCount
            records {
                __typename
                user_id
...
const RemoveWishlistItemMutation = gql(/* GraphQL */ `
    mutation RemoveWishlistItemMutation($productId: String, $userId: UUID) {
        deleteFromwishlistCollection(filter: { and: [{ user_id: { eq: $userId } }, { product_id: { eq: $productId } }] }, atMost: 1) {
            records {
                __typename
            }
        }
...
function AddToWishListButton({ productId }: Props) {
    ...
    return (
        <Button className="rounded-full p-3" variant="ghost" onClick={onClickHandler}>
            <Icons.heart className={cn("w-4 h-4", wishlist[productId] ? "fill-red-600 " : "fill-none")} />
        </Button>
    );
}
